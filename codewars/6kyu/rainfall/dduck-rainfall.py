...
def parse_to_dict(data):
    return dict(
        map(
            lambda row: (row[0], list(map(lambda x: float(x.split(" ")[1]), row[1].split(",")))),
            map(
...
def mean(town, data):
    parsed_value = parse_to_dict(data)

    if town not in parsed_value:
        return -1

    return sum(parsed_value[town]) / 12
...
def variance(town, data):
    ...
    mean_value = mean(town, data)
    return sum(map(lambda v: abs(mean_value - v) ** 2, parsed_value[town])) / 12
