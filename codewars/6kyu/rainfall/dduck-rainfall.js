...
const parseToMap = (data) => new Map(data.split('\n').map((v) => v.split(':')));

const mean = (town, data) => {
  const mapData = parseToMap(data);
  
  if (mapData.has(town)) {
    return mapData.get(town)
      .split(',')
      .reduce((a, c) => {
        return a + Number(c.split(' ')[1], 10);
      }, 0) / 12;
...
const variance = (town, data) => {
  const meanValue = mean(town, data);
  
  if (meanValue !== -1) {
    return parseToMap(data).get(town)
      .split(',')
      .reduce((a, c) => a + Math.abs(meanValue - Number(c.split(' ')[1])) ** 2, 0) / 12;
  }
