...
const getMatchedDataList = (town, strng) => {
  const matchedData = strng.split('\n').filter((item) => item.split(':')[0] === town)[0]
  const datas = matchedData?.split(':')[1]
  return datas?.split(',').map((item) => item.substring(4))
}

const mean = (town, strng) => {
  const matchedDataList = getMatchedDataList(town, strng)
  if (!matchedDataList || matchedDataList.length < 1) {
    return -1
  }
  return matchedDataList.reduce((acc, cur) => acc + Number(cur), 0) / matchedDataList.length
}
...
const variance = (town, strng) => {
  ...
  return matchedDataList.reduce((acc, cur) => {
    const deviation = avg - Number(cur)
    return acc + deviation ** 2 / matchedDataList.length
  }, 0)
}
