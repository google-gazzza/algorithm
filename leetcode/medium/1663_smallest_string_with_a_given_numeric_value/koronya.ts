...
// Runtime: 124 ms, faster than 100.00% of TypeScript online submissions for Smallest String With A Given Numeric Value.
// Memory Usage: 49.7 MB, less than 50.00% of TypeScript online submissions for Smallest String With A Given Numeric Value.

const CHAR_CODE_OF_A = 'a'.charCodeAt(0)
const LENGTH_OF_ALPHABET = 26

const getCharFromNumber = (num: number): string => {
  return String.fromCharCode(num - 1 + CHAR_CODE_OF_A)
}

const getString = (num: number, len: number): string => {
  let str = ''
  const limit = len - 1
